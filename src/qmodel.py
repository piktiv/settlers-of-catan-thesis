...
def q_model(input_shape, action_space):
    model = Sequential()
    model.add(Conv2D(filters=16, kernel_size=5, activation='relu', strides=1, input_shape=input_shape,
                     padding='same',
    ...
    model.add(Dense(len(action_space) + 1, activation='linear', kernel_initializer='he_normal'))
    model.add(Lambda(lmd))
    model.compile(loss="huber_loss", optimizer=RMSprop(learning_rate=0.00025, rho=0.95, epsilon=0.01),
                  metrics=["accuracy"])
    model.summary()
    ...
    # Output Layer
    x = Dense(len(action_space) + 1, activation="linear", kernel_initializer='he_normal')(x)
    output = Lambda(lmd)(x)
